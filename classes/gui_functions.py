...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.arduino1 = ArduinoHandler(self.tbprint)
        self.arduino1.connect(PORT1)
    ...
    def update_actions(self, actions, stopped, robot_list, cell_list):
        ...
        if self.control_status == True:
            ...
            type, self.Bx, self.By, self.Bz, self.alpha, self.gamma, self.freq, self.psi, _, self.Mx, self.My, self.Mz = self.controller_actions.run(self.joystick)
            self.psi = np.radians(self.ui.psidial.value())
            ...
            if self.actions_counter < self.excel_actions_df['Frame'].iloc[-1]:
                filtered_row = self.excel_actions_df[self.excel_actions_df['Frame'] == self.actions_counter].iloc[0]
                ...
                self.gradient_status = float(filtered_row["Gradient?"])
                self.equal_field_status = float(filtered_row["Equal Field?"])
                self.acoustic_frequency = float(filtered_row["Acoustic Frequency"])
