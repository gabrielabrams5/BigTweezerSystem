...
class VideoThread(QThread):
    ...
    def track_robot(self, frame, robotmask):
        ...
        if len(self.robot_list) > 0:
            # Iterate in reverse so lost-robot deletions don't skip subsequent indices.
            for i in range(len(self.robot_list) - 1, -1, -1):
                try:
            ...
            if len(self.robot_list)>0:
                latest_bot = self.robot_list[-1]
                x1_record = int(latest_bot.position_list[-1][0] - self.crop_length_record/2)
                y1_record = int(latest_bot.position_list[-1][1] - self.crop_length_record/2)
                ...
                latest_bot.crop_length = self.robot_crop_length
            else:
    ...
    def track_cell(self, frame, cellmask):
        ...
        if len(self.cell_list) > 0:
            # Iterate in reverse so lost-cell deletions don't skip subsequent indices.
            ...
            #adjust most recent cell crop_length
            if len(self.cell_list) > 0:
                self.cell_list[-1].crop_length = self.cell_crop_length
